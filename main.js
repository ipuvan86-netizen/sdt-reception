...
// ---------------------------------------------------------------------
// DENTICARE APPLICATIONS - website form items on the Home action list.
...
const REACT_RECALL_SMS = 'Hi, this is Southside Dental Toowoomba. We tried to give you a call today - we haven\'t seen you since {when} and you\'re now due for your check-up and clean. You\'re welcome to book online at www.sdtoowoomba.com.au, reply to this message, or call us on (07) 4635 1444. We\'d love to see you again soon!';
const REACT_RECALL_SMS_NODATE = 'Hi, this is Southside Dental Toowoomba. We tried to give you a call today - it\'s been a while since your last visit and you\'re now due for your check-up and clean. You\'re welcome to book online at www.sdtoowoomba.com.au, reply to this message, or call us on (07) 4635 1444. We\'d love to see you again soon!';

// "08/01/2025" -> "January 2025". Month + year reads warmer in a text than
// an exact date; anything unparseable falls back to the no-date wording.
function lastVisitWhen(s) {
...
function reactSmsFor(it) {
  ...
  if (it.kind === 'reactrecall') {
    const when = lastVisitWhen(it.lastVisit);
    return when ? REACT_RECALL_SMS.replace('{when}', when) : REACT_RECALL_SMS_NODATE;
  }
...
ipcMain.handle('react-send-sms', async (e, p) => {
  ...
  if (it.kind !== 'reactcdbs' && it.kind !== 'reactivation' && it.kind !== 'reactrecall') return { ok: false, error: 'In-app texting is only wired for the Reactivation lists.' };
  ...
  const smsTag = it.kind === 'reactcdbs' ? 'react-cdbs' : it.kind === 'reactrecall' ? 'react-recall' : 'react-hf';
  if (!r.ok) {
...
ipcMain.handle('action-pin-note', async (e, p) => {
  ...
  const it = a.items.find(x => x.id === p.id);
  if (!it) return { ok: false, error: 'Item not found.' };
  // Synthetic ids (name:, recall:, appt:) are bookkeeping keys, NOT Principle
  // patient ids - queueing a note against one silently writes nowhere.
  if (!it.patientId || /^(name:|recall:|appt:)/.test(String(it.patientId))) return { ok: false, error: 'No Principle patient link on this item.' };
  const draft = String(p.text || '').trim().slice(0, 300);
...
function loadAutoJobs() {
  ...
  if (!s.jobs.find(j => j.id === 'react-recall')) {
    s.jobs.push({
      id: 'react-recall',
      name: 'Reactivation recall — not seen 8-36 months',
      desc: 'Recall patients not seen for 8-36 months (the report excludes health fund/DVA patients and under-18s - they have their own lists). Cards land on the Reactivation Recall screen (left sidebar); the SMS says when they were last seen and that a check-up & clean is due. Rung patients stay off for 180 days.',
      url: 'https://app.principle.dental/reporting/custom-reports/1NTbTqWOrqtMuFsgYq8a',
      days: [1, 2, 3, 4, 5],
      time: '09:15',
      enabled: true,
      lastRun: null,
  ...
  if (!s.jobs.find(j => j.id === 'recall')) {
    s.jobs.push({
      ...
      desc: 'Patients whose recall is due but who have no recall appointment booked. Lands in the "No 6/12 rebooked" section of the Action list. Two ticks per item like the checkouts: the dentist confirms their part, reception books them in - both ticked moves it to Done, where it stays for good (the next visit-cycle makes a fresh item). If reception books them in Principle without ticking, the next run ticks the item off by itself.',
...
async function runRecallJob(job) {
  ...
  // The report is the COMPLETE set of patients whose recall is due, so an
  // empty report is a SUCCESS (nobody outstanding) - carry on with zero
  // rows so the mirror pass below can tick everything off.
  const rows = gen.empty ? [] : rowsToObjects(csvToRows(fs.readFileSync(gen.file).toString('utf8')));
  if (gen.empty) runlog('report returned 0 rows - nobody is due and unbooked');
  const headers = rows.length ? Object.keys(rows[0]) : [];
  ...
  runlog('report rows parsed: ' + rows.length + (rows.length && !linkKey ? ' (no Patient Link column - add it in the report builder to enable the Principle button)' : ''));
  ...
  const now = new Date().toISOString();
  let added = 0, healed = 0;
  // Tokens still genuinely OUTSTANDING today. A patient who has since been
  // booked comes back on the report with "has any recall" = Y, so they are
  // deliberately NOT added here - which is exactly what makes the mirror
  // pass below able to tick their item off.
  const seenTokens = new Set();
  for (const o of rows) {
    const name = (o[nameKey] || '').trim();
    if (!name) { runlog('  row skipped: blank patient name'); continue; }
    const link = linkKey ? (o[linkKey] || '').trim() : '';
    const realPid = (link && patientIdFromLink(link)) || null;
    ...
    const known = a.items.find(it => it.kind === 'recall' && it.token === token);
    if (known) {
      // Backfill older items minted before the real Principle id was kept.
      let touched = false;
      if (link && !known.plink) { known.plink = link; touched = true; }
      if (realPid && String(known.patientId || '').startsWith('recall:')) { known.patientId = realPid; touched = true; }
      if (touched) { known.updatedAt = now; await fsPush(known); healed++; }
      runlog('  "' + name + '" skipped: already on the list (' + token + ')');
      continue;
    }
    const it = {
      id: 'a' + Date.now() + Math.random().toString(36).slice(2, 6),
      // The real Principle patient id when the report gives us the link -
      // that is what makes the "-> Principle" note button work. Without a
      // link we fall back to name:, which correctly greys the button out.
      patientId: realPid || ('name:' + name),
      name, kind: 'recall', section: 'No 6/12 rebooked',
  ...
  // ---- mirror pass ----
  // An open item whose token is no longer outstanding on the report means
  // the patient has since been booked in (or has dropped off the report
  // entirely). Tick it off automatically with an audit note - recoverable
  // via undo in Done, exactly like Complete notes and Huddle tags.
  let cleared = 0;
  for (const it of a.items) {
    if (it.doneAt || it.kind !== 'recall') continue;
    if (seenTokens.has(it.token)) continue;                      // still outstanding - genuinely open
    it.doneAt = now; it.doneNote = 'auto-cleared: recall now booked in Principle (off the report)'; it.auto = true;
    it.updatedAt = now;
    await fsPush(it);
    cleared++;
    runlog('  "' + it.name + '" auto-cleared: recall now booked');
  }
  if (cleared > 40) appJournal('No 6/12 rebooked: unusually large auto-clear (' + cleared + ' items) - if that looks wrong, check the report and use undo in Done');
  // ---- rename migration: "Recalls" -> "Not rebooked" ----
  // Two different things in this app were called recall (this list, and
  // the 8-36 month Reactivation Recall screen). Harmless to repeat: it
  // only acts on items still carrying the old section name.
  // Only items still on screen are rewritten - open ones, plus anything
  // done inside the 14-day Done window (30 for margin). Long-dead records
  // keep the old name harmlessly and stay dedupe-valid (the token, not the
  // section, is their identity), which keeps the first run's write burst
  // small instead of rewriting years of history in one go.
  let renamed = 0;
  for (const it of a.items) {
    if (it.kind !== 'recall' || it.section === 'No 6/12 rebooked') continue;
    if (it.doneAt && daysSince(it.doneAt) > 30) continue;
    it.section = 'No 6/12 rebooked';
    it.updatedAt = now;
    await fsPush(it);
    renamed++;
  }
  if (renamed) runlog('section rename: ' + renamed + ' item(s) moved to "No 6/12 rebooked"');
  // Carry any audience rule across with the name, so nobody's view of this
  // section silently changes on the day of the rename.
  try {
    const cfg = a.items.find(x => x.id === '_viewsConfig');
    if (cfg && cfg.viewsRules) {
      const rules = JSON.parse(cfg.viewsRules);
      let carried = 0;
      if (rules['Recalls'] && !rules['No 6/12 rebooked']) { rules['No 6/12 rebooked'] = rules['Recalls']; delete rules['Recalls']; carried++; }
      if (rules['Not rebooked'] && !rules['No 6/12 rebooked']) { rules['No 6/12 rebooked'] = rules['Not rebooked']; delete rules['Not rebooked']; carried++; }
      if (carried) {
        cfg.viewsRules = JSON.stringify(rules);
        await fsPush(cfg);
        runlog('views: audience rule carried across to "No 6/12 rebooked"');
      }
    }
  } catch (eV) { runlog('views rule rename skipped: ' + ((eV && eV.message) || eV)); }
  saveActions(a);
  runlog('recalls not booked: ' + added + ' item(s) added'
    + (cleared ? ', ' + cleared + ' auto-cleared' : '')
    + (healed ? ', ' + healed + ' Principle link(s) backfilled' : ''));
  sendUi('actions-changed', {});
...
async function runNoNextVisitJob(job) {
  ...
  for (const o of rows) {
    ...
    seenKeys.add(pid + '|' + token);
    // Any item for this patient+visit - open, parked, or Done at any age -
    // suppresses re-minting. Done means handled for this visit-cycle; only
    // a new visit without a booking makes a fresh item (like the recall list).
    const dup = a.items.some(it => it.patientId === pid && it.kind === 'rebook' && it.token === token);
    if (dup) { runlog('  "' + name + '" skipped: already on the list'); continue; }
    const mob = mobKey ? (o[mobKey] || '').trim() : '';
    const it = {
      id: 'a' + Date.now() + Math.random().toString(36).slice(2, 6),
      patientId: pid, name, kind: 'rebook', section: 'Rebook (dentist check)',
  ...
  // ---- mirror pass ----
  // Open OR parked Rebook items whose patient+visit is no longer on the
  // report are ticked off automatically: they either booked in, or they
  // left the report's 30-day window. Recoverable via undo in Done. A
  // parked patient who visits again without booking mints a fresh item.
  let cleared = 0;
  for (const it of a.items) {
    if (it.doneAt || it.kind !== 'rebook') continue;
    if (seenKeys.has((it.patientId || '') + '|' + (it.token || ''))) continue;   // still on the report - genuinely open
    it.doneAt = now; it.doneNote = 'auto-cleared: off the no-next-visit report (booked, or 30-day window passed)'; it.auto = true;
    it.updatedAt = now;
    await fsPush(it);
    cleared++;
    runlog('  "' + it.name + '" auto-cleared: off the no-next-visit report');
  }
  if (cleared > 40) appJournal('Rebook (dentist check): unusually large auto-clear (' + cleared + ' items) - if that looks wrong, check the report and use undo in Done');
  // ---- rename migration: "Rebook" -> "Rebook (dentist check)" ----
  // Harmless to repeat: only touches items still carrying the old name.
  let renamed = 0;
  for (const it of a.items) {
    if (it.kind !== 'rebook' || it.section === 'Rebook (dentist check)') continue;
    if (it.doneAt && daysSince(it.doneAt) > 30) continue;
    it.section = 'Rebook (dentist check)';
    it.updatedAt = now;
    await fsPush(it);
    renamed++;
  }
  if (renamed) runlog('section rename: ' + renamed + ' item(s) moved to "Rebook (dentist check)"');
  try {
    const cfg = a.items.find(x => x.id === '_viewsConfig');
    if (cfg && cfg.viewsRules) {
      const rules = JSON.parse(cfg.viewsRules);
      if (rules['Rebook'] && !rules['Rebook (dentist check)']) {
        rules['Rebook (dentist check)'] = rules['Rebook'];
        delete rules['Rebook'];
        cfg.viewsRules = JSON.stringify(rules);
        await fsPush(cfg);
        runlog('views: audience rule carried across to "Rebook (dentist check)"');
      }
    }
  } catch (eV) { runlog('views rule rename skipped: ' + ((eV && eV.message) || eV)); }
  saveActions(a);
  runlog('no next visit: ' + added + ' item(s) added' + (cleared ? ', ' + cleared + ' auto-cleared' : ''));
  sendUi('actions-changed', {});
  return { outcome: added + ' item(s) added' + (cleared ? ', ' + cleared + ' auto-cleared' : ''), added };
}
...
async function runUnpaidJob(job) {
  ...
  const remKey = key(/amount remaining/i), owingKey = key(/total outstanding/i), feeKey = key(/fee schedule/i);
  ...
  for (const o of rows) {
    ...
    // This invoice's true figure: Amount Remaining when the report has it
    // (part-payments make Invoice Total quietly wrong), else fall back.
    ...
    // Lifecycle: open item -> freshen its figures from today's report, skip.
    // Ticked within 50 days -> snoozed, skip.
    // Ticked 50+ days ago and STILL on the report -> escalate it red.
    const known = a.items.find(it => it.kind === 'unpaid' && it.token === token);
    if (known) {
      if (!known.doneAt) {
        const wantOwed = owedTotal || undefined, wantFee = feeSched || undefined;
        if (known.context !== freshCtx || known.owedTotal !== wantOwed || known.feeSched !== wantFee) {
          known.context = freshCtx;
          if (wantOwed) known.owedTotal = wantOwed; else delete known.owedTotal;
          if (wantFee) known.feeSched = wantFee; else delete known.feeSched;
          known.updatedAt = now;
          await fsPush(known);
          runlog('  "' + name + '" already open - figures freshened from today\'s report');
        } else runlog('  "' + name + '" skipped: already open');
        continue;
      }
    ...
    if (owedTotal) it.owedTotal = owedTotal;
    if (feeSched) it.feeSched = feeSched;
    a.items.push(it);
    await fsPush(it);
    added++;
  }
...
// Recall reactivation: patients not seen for 8-36 months, from its own
// Principle report (which excludes health-fund/DVA patients and under-18s
// in its filters - those live on their own lists). Identical card life to
// the other two lists: ring, outcome, 180 days off after a ring.
async function runReactRecallJob(job) {
  runlogStart('auto-react-recall');
  runlog('=== auto report: ' + job.name + ' ===');
  const login = await ensurePrincipleForJobs();
  if (!login.ok) { runlog('Principle not available - job skipped'); return { outcome: 'skipped: Principle needs a login (window opened)' }; }
  const gen = await principleReport.generateReport(null, (t) => runlog('  ' + t), job.url);
  ...
  const headers = rows.length ? Object.keys(rows[0]) : [];
  const key = (re) => headers.find(h => re.test(h));
  const nameKey = key(/patient name/i), mobKey = key(/mobile/i), lastKey = key(/last visit/i), linkKey = key(/patient link/i);
  if (!nameKey) { runlog('no Patient Name column'); return { outcome: 'failed: no Patient Name column' }; }
  runlog('report rows parsed: ' + rows.length + (linkKey ? '' : ' (no Patient Link column - add it in the report builder to enable the Principle button and note-writing)'));
  ...
  for (const o of rows) {
    ...
    const link = linkKey ? (o[linkKey] || '').trim() : '';
    const pid = (link && patientIdFromLink(link)) || ('name:' + name);
    const token = 'reactrecall:' + pid;
    const known = a.items.find(it => it.kind === 'reactrecall' && it.token === token);
    if (known) {
      if (link && !known.plink) { known.plink = link; known.patientId = pid; fsPush(known); }   // backfill
      if (!known.doneAt) continue;                                        // open (or follow-up scheduled)
      if (daysSince(known.doneAt) < 180) continue;                        // rung within 6 months
      delete known.doneAt; delete known.outcome; known.attempts = '';
      known.text = 'Ring for their check-up & clean (6+ months since last call)';
      known.updatedAt = now;
      fsPush(known); reopened++;
      continue;
    }
    const it = {
      id: 'a' + Date.now() + Math.random().toString(36).slice(2, 6),
      patientId: pid, name, kind: 'reactrecall', section: 'Reactivation Recall',
      text: 'Ring for their check-up & clean',
      context: '',
      mobile: mobKey ? (o[mobKey] || '').trim() : '',
      lastVisit: lastKey ? (o[lastKey] || '').trim() : '',
      plink: link || '',
  ...
  saveActions(a);
  runlog('recall reactivation: ' + added + ' added, ' + reopened + ' reopened after 180 days');
  sendUi('actions-changed', {});
  return { outcome: added + ' added' + (reopened ? ', ' + reopened + ' reopened (180d)' : ''), added };
}
...
async function runAutoJob(id) {
  ...
  try {
    ...
    else if (job.id === 'react-recall') result = await runReactRecallJob(job);
