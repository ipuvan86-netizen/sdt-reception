...
async function runRecallJob(job) {
  ...
  for (const o of rows) {
    ...
    const it = {
      ...
      name, kind: 'recall', section: 'Not rebooked',
  ...
  if (cleared > 40) appJournal('Not rebooked: unusually large auto-clear (' + cleared + ' items) - if that looks wrong, check the report and use undo in Done');
  ...
  let renamed = 0;
  for (const it of a.items) {
    if (it.kind !== 'recall' || it.section === 'Not rebooked') continue;
    if (it.doneAt && daysSince(it.doneAt) > 30) continue;
    it.section = 'Not rebooked';
    it.updatedAt = now;
    await fsPush(it);
    renamed++;
  }
  if (renamed) runlog('section rename: ' + renamed + ' item(s) moved from "Recalls" to "Not rebooked"');
  ...
  try {
    ...
    if (cfg && cfg.viewsRules) {
      ...
      if (rules['Recalls'] && !rules['Not rebooked']) {
        rules['Not rebooked'] = rules['Recalls'];
        delete rules['Recalls'];
        cfg.viewsRules = JSON.stringify(rules);
        await fsPush(cfg);
        runlog('views: audience rule carried across from "Recalls" to "Not rebooked"');
      }
    }
